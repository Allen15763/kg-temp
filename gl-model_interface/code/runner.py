...
class Runner:
    ...
    def __init__(self, run_name: str, model_cls: Callable[[str, dict], Model], params: dict, train=True):
        """建構子
        ...
        if train:
            self.tr_x, self.tr_y = self.load_train()
            self.features = self.tr_x.columns.tolist()
        else:
            self.va_x = self.load_x_test()
        self.params = params
        self.n_fold = 4
    ...
    def train_fold(self, i_fold: Union[int, str]) -> Tuple[
        ...
        train_x, train_y = self.tr_x, self.tr_y
    ...
    def load_train(self) -> pd.DataFrame:
        ...
        train_y = df['target']
        lk = LabelEncoder()
        train_y = lk.fit_transform(train_y)
        self.save_encoder(lk)
        train_y = pd.Series(train_y)
        return train_x, train_y
    
    def save_encoder(self, ob):
        with open("../model/target.plk", 'wb') as f:
            pickle.dump(ob, f)

    def load_x_test(self) -> pd.DataFrame:
        return pd.read_csv('../input/test.csv').iloc[:, :-1]
